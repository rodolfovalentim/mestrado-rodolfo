...
ovs-ofctl -O OpenFlow13 add-group --may-create br-ex group_id=1,type=select,bucket=group:2,weight=1,bucket=group:3,weight=1
ovs-ofctl -O OpenFlow13 add-group --may-create  br-ex group_id=2,type=fast_failover,bucket=watch_port:1,actions="resubmit(,1)",bucket=watch_port:2,actions="resubmit(,2)"
ovs-ofctl -O OpenFlow13 add-group --may-create br-ex group_id=3,type=fast_failover,bucket=watch_port:2,actions="resubmit(,2)",bucket=watch_port:1,actions="resubmit(,1)"

echo "===== Installing WatchDog Flows ====="
ovs-ofctl -O OpenFlow13 del-flows br-ex
ovs-ofctl -O OpenFlow13 add-flow br-ex "table=0, priority=10, ip, in_port=3, actions=group:1"
ovs-ofctl -O OpenFlow13 add-flow br-ex "table=0, priority=100, icmp, nw_dst=8.8.8.8, in_port=3, actions=resubmit(,1)"
ovs-ofctl -O OpenFlow13 add-flow br-ex "table=0, priority=100, icmp, nw_dst=8.8.4.4, in_port=3, actions=resubmit(,2)"
ovs-ofctl -O OpenFlow13 add-flow br-ex "table=0, priority=0,actions=output:NORMAL"

echo "===== Getting WAN1 info ====="
OFFERED_IP_WAN1="172.16.30.70"
HEX_OFFERED_IP_WAN1=`python -c "print ''.join([hex(int(x)+256)[3:] for x in '$OFFERED_IP_WAN1'.split('.')])"`
ROUTER_IP_WAN1="172.16.30.71"
SUBNET_WAN1="255.255.255.0"

if [ "$OFFERED_IP_WAN1" ];
then 
    # MAC_GW_WAN1=$(arping -I $WAN1 -s 0.0.0.0 -f $ROUTER_IP_WAN1 | grep "reply" | echo $(cut -d' ' -f5) | tail -c +2 | head -c -2)
    MAC_GW_WAN1="00:26:2d:66:87:13"
    echo "=== WAN 1 ==="
    ...
    ovs-ofctl -O OpenFlow13 add-flow br-ex "table=1, ip,action=ct(commit,zone=1,nat(src=$OFFERED_IP_WAN1)),mod_dl_src=$MACETH1,mod_dl_dst=$MAC_GW_WAN1,output=1" 
    ovs-ofctl -O OpenFlow13 add-flow br-ex "table=10, priority=100,ct_state=-trk,ip,action=ct(table=10,zone=1,nat)"
    ovs-ofctl -O OpenFlow13 add-flow br-ex "table=10, priority=100,ct_state=+est,ct_zone=1,ip,action=mod_dl_src=$DLSRC,mod_dl_dst=$DLDST, output=3"
    ovs-ofctl -O OpenFlow13 add-flow br-ex "table=0, priority=100,in_port=1,actions=resubmit(,10)"
    ovs-ofctl -O OpenFlow13 add-flow br-ex "table=0, arp, in_port=1, nw_dst=$OFFERED_IP_WAN1 \
                                            actions= \
                                            ...
                                            load:0x$HEX_OFFERED_IP_WAN1->NXM_OF_ARP_SPA[], in_port"
else
    echo "=== WAN 1 not UP ==="
fi

# OFFERED_IP_WAN2="172.16.30.71"
# HEX_OFFERED_IP_WAN2=`python -c "print ''.join([hex(int(x)+256)[3:] for x in '$OFFERED_IP_WAN2'.split('.')])"`
# ROUTER_IP_WAN2="172.16.31.254"
# SUBNET_WAN2="255.255.254.0"
...
then 
    ...
    ovs-ofctl -O OpenFlow13 add-flow br-ex "table=2, ip,action=ct(commit,zone=2,nat(src=$OFFERED_IP_WAN2)),mod_dl_src=$MACETH2, mod_dl_dst=$MAC_GW_WAN2, output=2"
    ovs-ofctl -O OpenFlow13 add-flow br-ex "table=20, priority=100,ct_state=-trk,ip,action=ct(table=20,zone=2,nat)"
    ovs-ofctl -O OpenFlow13 add-flow br-ex "table=20, priority=100,ct_state=+est,ct_zone=2,ip,action=mod_dl_src=$DLSRC,mod_dl_dst=$DLDST, output=3"
    ovs-ofctl -O OpenFlow13 add-flow br-ex "table=0, priority=100,in_port=2,actions=resubmit(,20)"
    ovs-ofctl -O OpenFlow13 add-flow br-ex "table=0, arp, in_port=2, nw_dst=$OFFERED_IP_WAN2 \
                                            actions= \
                                            move:NXM_OF_ETH_SRC[]->NXM_OF_ETH_DST[], \
                                            mod_dl_src=$MACETH2, \
                                            load:0x2->NXM_OF_ARP_OP[], \
                                            ...
                                            load:0x$HEX_OFFERED_IP_WAN2->NXM_OF_ARP_SPA[], in_port"
